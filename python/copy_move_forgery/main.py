...
def getparts(image, block_len):
    ...
    global opt
    for n in range(int(8)):
        img = img.filter(ImageFilter.SMOOTH_MORE)
    # Converting image to custom palette
    pal = [x for x in range(256) if x % int(30) == 0]
    if pal[-1] != 255:
        pal.append(255)
    imagetopalette(img, pal)
    pix = img.load()
    width_image = w - block_len
    height_image = h - block_len
...
def similarparts(imagparts):
    ...
    global opt
    l = len(imagparts[0])-1
    length = len(imagparts)-1

    for i in range(length):
        difs = sum(abs(x-y)
                   for x, y in list(zip(imagparts[i][:l], imagparts[i+1][:l])))
        mean = float(sum(imagparts[i][:l])) / l
        dev = float(sum(abs(mean-val) for val in imagparts[i][:l])) / l
        if dev/mean >= float(0.05):
            if difs <= int(100):
...
def clusterparts(parts, block_len):
    ...
    clusters = [clust for clust in clusters 
        if Dist((min(clust, key=item(0))[0], min(clust, key=item(1))[1]), (max(clust, key=item(0))[0], max(clust, key=item(1))[1]))/(block_len*1.4) >= float(0.5)
    ]

    # filter out clusters, which doesn`t have identical twin cluster
    clusters = [clust for x, clust in enumerate(
        clusters) if hassimilarcluster(x, clusters)]
...
def marksimilar(image, clust, size):
    ...
    if clust:
        ...
        mask = Image.new('RGB', (size, size), 'red')
        for cl in clust:
            for x, y in cl:
...
if __name__ == '__main__':
    ...
    cmd.add_option(
        '', '--imauto', help='Automatically search identical regions. (default: %default)', default=1)
    cmd.add_option(
        '', '--imblev', help='Blur level for degrading image details. (default: %default)', default=8)
    cmd.add_option(
        '', '--impalred', help='Image palette reduction factor. (default: %default)', default=15)
    cmd.add_option(
    ...
    cmd.add_option(
        '', '--blcoldev', help='Block color deviation threshold. (default: %default)', default=0.2)
    cmd.add_option(
    ...
    print(dparts)
    print("dparts")
    cparts = clusterparts(dparts, block_len) if 1 else [
        [elem[-1] for elem in dparts]]
    print("cparts")
    im = marksimilar(im, cparts, block_len)
    out = "dune_doc.png".split('.')[0] + '_analyzed.png'
    im.save(out)
    print('Done. Found', len(cparts) if 1 else 0, 'identical regions')
